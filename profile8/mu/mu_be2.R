...
idx <- setdiff(1:ncol(box), grep("mu_be", colnames(box)))
profile_design(
  mu_be1=seq(0.72,0.9999,length=profile_pts),
...
bake(file =paste0(out_dir,"profile_mu_be2.rds"), {
  
  cat(capture.output(sessionInfo()),
      file=paste0(out_dir,"sessionInfo_mu_be2.txt"),sep="\n")
