...
idx <- setdiff(1:ncol(box), grep("mu_af", colnames(box)))
profile_design(
  mu_af1=seq(0.15,0.925,length=profile_pts),
...
bake(file =paste0(out_dir,"profile_mu_af2.rds"), {
  
  cat(capture.output(sessionInfo()),
      file=paste0(out_dir,"sessionInfo_mu_af2.txt"),sep="\n")
