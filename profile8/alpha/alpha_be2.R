...
mle <- unlist(params_full[which.max(params_full$logLik),-c(1,2)])
mle <- mle[names(mle)%in%names(coef(li23(U = 2, for_ibpf = F)))] # from for_ibpf=T to for_ibpf=F
...
idx <- setdiff(1:ncol(box), grep("alpha_be", colnames(box)))
profile_design(
  alpha_be1=seq(0.001,0.18,length=profile_pts),
...
bake(file =paste0(out_dir,"profile_alpha_be2.rds"), {
  
  cat(capture.output(sessionInfo()),
      file=paste0(out_dir,"sessionInfo_alpha_be2.txt"),sep="\n")
