...
idx <- setdiff(1:ncol(box), grep("alpha_af", colnames(box)))
profile_design(
  alpha_af1=seq(0.17,0.9999,length=profile_pts),
...
bake(file =paste0(out_dir,"profile_alpha_af.rds"), {
  
  cat(capture.output(sessionInfo()),
      file=paste0(out_dir,"sessionInfo_alpha_af.txt"),sep="\n")
