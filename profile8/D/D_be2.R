...
idx <- setdiff(1:ncol(box), grep("D_be", colnames(box)))
profile_design(
  D_be1=seq(0.1,60,length=profile_pts),
...
bake(file =paste0(out_dir,"profile_D_be2.rds"), {
  
  cat(capture.output(sessionInfo()),
      file=paste0(out_dir,"sessionInfo_D_be2.txt"),sep="\n")
