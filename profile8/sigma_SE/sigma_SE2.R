...
idx <- setdiff(1:ncol(box), grep("sigma_SE", colnames(box)))
profile_design(
  sigma_SE1=seq(0.9,2.93,length=profile_pts),
...
bake(file =paste0(out_dir,"profile_sigma_SE2.rds"), {
  
  cat(capture.output(sessionInfo()),
      file=paste0(out_dir,"sessionInfo_sigma_SE2.txt"),sep="\n")
