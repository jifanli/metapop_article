...
li23_spatPomp <- stew(file=paste0(li23_model_dir,"li23_spatPomp7.rda"),{
...
stew(file=paste0(files_dir,"settings7.rda"),{
...
spatPomp <- stew(file=paste0(files_dir,"spatPomp7.rda"),{
  load(file=paste0(li23_model_dir,"li23_spatPomp7.rda"))  
...
girf <- stew(file=paste0(files_dir,"girf7.rda"),seed=5981724,{
...
abf <- stew(file=paste0(files_dir,"abf7.rda"),seed=844424,{
...
ubf <- stew(file=paste0(files_dir,"ubf7.rda"),seed=844425,{
  foreach(job=iter(jobs,"row")) %do% {
    system.time(
      abf(li23_list[[job$U_id]], 
          Nrep = ubf_Nrep,
...
abfir <- stew(file=paste0(files_dir,"abfir7.rda"),seed=53398,{
  foreach(job=iter(jobs,"row")) %do% {
    system.time(
...
pfilter <- stew(file=paste0(files_dir,"pfilter7.rda"),seed=53286,{
  foreach(job=iter(jobs,"row")) %dopar% {
    system.time(
...
bpf <- stew(file=paste0(files_dir,"bpf7.rda"),seed=53285,{
  foreach(job=iter(jobs,"row")) %dopar% {
    system.time(
...
list_enkf <- li23_list
enkf <- stew(file=paste0(files_dir,"enkf7.rda"),seed=53285,{
  foreach(job=iter(jobs,"row")) %dopar% {
...
save(file=paste0(files_dir,"results7.rda"),results,jobs)
...
filter_tests <- ggplot(results,mapping = aes(x = U, y = logLik_per_obs, group=Method,color=Method,linetype=Method,shape=Method)) +
  ...
  coord_cartesian(ylim=c(max-10,max))+
...
ggsave("filter_tests7.png", plot=filter_tests, width=10, height=5, dpi=300)
