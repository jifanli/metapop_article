...
li23_spatPomp <- stew(file=paste0(li23_model_dir,"li23_spatPomp10v9.rda"),{
  out <- readRDS("l8.rds")
...
stew(file=paste0(files_dir,"settings10v9.rda"),{
...
spatPomp <- stew(file=paste0(files_dir,"spatPomp10v9.rda"),{
  load(file=paste0(li23_model_dir,"li23_spatPomp10v9.rda"))  
...
girf <- stew(file=paste0(files_dir,"girf10v9.rda"),seed=5981724,{
...
abf <- stew(file=paste0(files_dir,"abf10v9.rda"),seed=844424,{
...
ubf <- stew(file=paste0(files_dir,"ubf10v9.rda"),seed=844425,{
  foreach(job=iter(jobs,"row")) %do% {
    system.time(
      abf(li23_list[[job$U_id]], 
          Nrep = ubf_Nrep,
...
abfir <- stew(file=paste0(files_dir,"abfir10v9.rda"),seed=53398,{
  foreach(job=iter(jobs,"row")) %do% {
    system.time(
...
pfilter <- stew(file=paste0(files_dir,"pfilter10v9.rda"),seed=53286,{
  foreach(job=iter(jobs,"row")) %dopar% {
    system.time(
...
bpf <- stew(file=paste0(files_dir,"bpf10v9.rda"),seed=53285,{
  foreach(job=iter(jobs,"row")) %dopar% {
    system.time(
...
list_enkf <- li23_list
enkf <- stew(file=paste0(files_dir,"enkf10v9.rda"),seed=53285,{
  foreach(job=iter(jobs,"row")) %dopar% {
...
save(file=paste0(files_dir,"results10v9.rda"),results,jobs)
...
ggsave("filter_tests10v9.png", plot=filter_tests, width=10, height=8, dpi=300)
