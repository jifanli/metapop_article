...
li23_spatPomp <- bake(file=paste0(li23_model_dir,"li23_spatPomp2.rds"),{
  li23_U <- 373
  li23_N <- 30
  li23_mod <- li23(U = li23_U, for_ibpf = F)
  ...
  li23_params <- li23_params[names(li23_params)%in%names(coef(li23_mod))] # from for_ibpf=T to for_ibpf=F
  set.seed(324739)
  li23_sim <- simulate(li23_mod,params=li23_params,
                       times=time(li23_mod)[1:li23_N])
  
  li23_subset <- function(m_U,m_N){
    m <- li23(U = m_U, for_ibpf = F)
    m@data <- li23_sim@data[1:m_U,1:m_N]
    time(m) <- li23_sim@times[1:m_N]
    coef(m) <- li23_params
    return(m)
  }
})
...
bake(file=paste0(files_dir,"settings2.rds"),{
  
  # copy variables that should be included in the bake
  run_level <- run_level 
  cores <- cores
  ...
  if(run_level==1){
    ...
    ubf_Nrep <- 20
    abfir_Nrep <- abf_Nrep
    abfir_Np_per_replicate <- abf_Np_per_replicate
    enkf_Np <- 10
...
spatPomp <- bake(file=paste0(files_dir,"spatPomp2.rds"),{
  list <- vector("list", length = length(U))  
  for(i in 1:length(U)) list[[i]] <- li23_subset(m_U=U[i], m_N=N)
})
...
# girf <- bake(file=paste0(files_dir,"girf2.rds"),seed=5981724,{
...
abf <- bake(file=paste0(files_dir,"abf2.rds"),seed=844424,{
...
ubf <- bake(file=paste0(files_dir,"ubf2.rds"),seed=844424,{
  foreach(job=iter(jobs,"row")) %do% {
    system.time(
      abf(list[[job$U_id]], 
          Nrep = ubf_Nrep,
...
# abfir <- bake(file=paste0(files_dir,"abfir2.rds"),seed=53398,{
#   foreach(job=iter(jobs,"row")) %do% {
#     system.time(
#       abfir(list[[job$U_id]], 
...
pfilter <- bake(file=paste0(files_dir,"pfilter2.rds"),seed=53285,{
  foreach(job=iter(jobs,"row")) %dopar% {
    system.time(
      pfilter(list[[job$U_id]],Np=pfilter_Np) -> pfilter_out
    ) -> pfilter_time
      #    uncomment for debugging 
...
bpf <- bake(file=paste0(files_dir,"bpf2.rds"),seed=53285,{
  foreach(job=iter(jobs,"row")) %dopar% {
    system.time(
      bpfilter(list[[job$U_id]],
               Np=bpf_Np,
...
list_enkf <- list
enkf <- bake(file=paste0(files_dir,"enkf2.rds"),seed=53285,{
  foreach(job=iter(jobs,"row")) %dopar% {
    list_enkf[[job$U_id]] <- spatPomp(list[[job$U_id]],
                                      vunit_measure=spatPomp_Csnippet(
                                        unit_paramnames='tau',
                                        code="
                                      double m = C>0 ? C : 0;
                                      double vtol = 1e-5;
                                      vc = m*tau[0]*m*tau[0]+m+vtol > 4 ? m*tau[0]*m*tau[0]+m+vtol : 4;
                                      ")
    )
    system.time(
      enkf(list_enkf[[job$U_id]],Np=enkf_Np) -> enkf_out
    ) -> enkf_time
    #    uncomment for debugging 
    #    list(logLik=logLik(enkf_out),time=enkf_time["elapsed"],enkf_out)
    list(logLik=logLik(enkf_out),time=enkf_time["elapsed"])
  } -> enkf_list
})
...
save(file=paste0(files_dir,"results2.rds"),results,jobs)
...
filter_tests <- ggplot(results,mapping = aes(x = U, y = logLik_per_obs, group=Method,color=Method,linetype=Method,shape=Method)) +
  ...
  coord_cartesian(ylim=c(max-500,max))+
...
ggsave("filter_tests2.png", plot=filter_tests, width=10, height=5, dpi=300)
