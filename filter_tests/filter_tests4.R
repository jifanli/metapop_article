...
li23_spatPomp <- stew(file=paste0(li23_model_dir,"li23_spatPomp4.rda"),{
...
stew(file=paste0(files_dir,"settings4.rda"),{
...
spatPomp <- stew(file=paste0(files_dir,"spatPomp4.rda"),{
  load(file=paste0(li23_model_dir,"li23_spatPomp4.rda"))  
...
# girf <- stew(file=paste0(files_dir,"girf4.rda"),seed=5981724,{
...
abf <- stew(file=paste0(files_dir,"abf4.rda"),seed=844424,{
...
ubf <- stew(file=paste0(files_dir,"ubf4.rda"),seed=844424,{
  foreach(job=iter(jobs,"row")) %do% {
    system.time(
      abf(li23_list[[job$U_id]], 
          Nrep = ubf_Nrep,
...
# abfir <- stew(file=paste0(files_dir,"abfir4.rda"),seed=53398,{
#   foreach(job=iter(jobs,"row")) %do% {
#     system.time(
...
pfilter <- stew(file=paste0(files_dir,"pfilter4.rda"),seed=53285,{
  foreach(job=iter(jobs,"row")) %dopar% {
    system.time(
...
bpf <- stew(file=paste0(files_dir,"bpf4.rda"),seed=53285,{
  foreach(job=iter(jobs,"row")) %dopar% {
    system.time(
...
list_enkf <- li23_list
enkf <- stew(file=paste0(files_dir,"enkf4.rda"),seed=53285,{
  foreach(job=iter(jobs,"row")) %dopar% {
...
save(file=paste0(files_dir,"results4.rda"),results,jobs)
...
ggsave("filter_tests4.png", plot=filter_tests, width=10, height=5, dpi=300)
