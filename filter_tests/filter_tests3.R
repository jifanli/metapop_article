...
li23_spatPomp <- stew(file=paste0(li23_model_dir,"li23_spatPomp3.rda"),{
  li23_U <- 373
  li23_N <- 30
...
stew(file=paste0(files_dir,"settings3.rda"),{
...
spatPomp <- stew(file=paste0(files_dir,"spatPomp3.rda"),{
  load(file=paste0(li23_model_dir,"li23_spatPomp3.rda"))  
...
# girf <- stew(file=paste0(files_dir,"girf3.rda"),seed=5981724,{
...
abf <- stew(file=paste0(files_dir,"abf3.rda"),seed=844424,{
...
ubf <- stew(file=paste0(files_dir,"ubf3.rda"),seed=844424,{
  foreach(job=iter(jobs,"row")) %do% {
    system.time(
      abf(li23_list[[job$U_id]], 
          Nrep = ubf_Nrep,
...
# abfir <- stew(file=paste0(files_dir,"abfir3.rda"),seed=53398,{
#   foreach(job=iter(jobs,"row")) %do% {
#     system.time(
...
pfilter <- stew(file=paste0(files_dir,"pfilter3.rda"),seed=53285,{
  foreach(job=iter(jobs,"row")) %dopar% {
    system.time(
...
bpf <- stew(file=paste0(files_dir,"bpf3.rda"),seed=53285,{
  foreach(job=iter(jobs,"row")) %dopar% {
    system.time(
...
list_enkf <- li23_list
enkf <- stew(file=paste0(files_dir,"enkf3.rda"),seed=53285,{
  foreach(job=iter(jobs,"row")) %dopar% {
...
save(file=paste0(files_dir,"results3.rda"),results,jobs)
...
ggsave("filter_tests3.png", plot=filter_tests, width=10, height=5, dpi=300)
