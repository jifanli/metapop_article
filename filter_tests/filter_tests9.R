...
li23_spatPomp <- stew(file=paste0(li23_model_dir,"li23_spatPomp9.rda"),{
  ...
  li23_params["Td_af1"] <- 6
  li23_params["Td_be1"] <- 9
...
stew(file=paste0(files_dir,"settings9.rda"),{
...
spatPomp <- stew(file=paste0(files_dir,"spatPomp9.rda"),{
  load(file=paste0(li23_model_dir,"li23_spatPomp9.rda"))  
...
girf <- stew(file=paste0(files_dir,"girf9.rda"),seed=5981724,{
...
abf <- stew(file=paste0(files_dir,"abf9.rda"),seed=844424,{
...
ubf <- stew(file=paste0(files_dir,"ubf9.rda"),seed=844425,{
  foreach(job=iter(jobs,"row")) %do% {
    system.time(
      abf(li23_list[[job$U_id]], 
          Nrep = ubf_Nrep,
...
abfir <- stew(file=paste0(files_dir,"abfir9.rda"),seed=53398,{
  foreach(job=iter(jobs,"row")) %do% {
    system.time(
...
pfilter <- stew(file=paste0(files_dir,"pfilter9.rda"),seed=53286,{
  foreach(job=iter(jobs,"row")) %dopar% {
    system.time(
...
bpf <- stew(file=paste0(files_dir,"bpf9.rda"),seed=53285,{
  foreach(job=iter(jobs,"row")) %dopar% {
    system.time(
...
list_enkf <- li23_list
enkf <- stew(file=paste0(files_dir,"enkf9.rda"),seed=53285,{
  foreach(job=iter(jobs,"row")) %dopar% {
...
save(file=paste0(files_dir,"results9.rda"),results,jobs)
...
filter_tests <- ggplot(results,mapping = aes(x = U, y = logLik_per_obs, group=Method,color=Method,linetype=Method,shape=Method)) +
  ...
  coord_cartesian(ylim=c(max-5,max))+
...
ggsave("filter_tests9.png", plot=filter_tests, width=10, height=5, dpi=300)
