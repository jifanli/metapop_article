...
li23_spatPomp <- stew(file=paste0(li23_model_dir,"li23_spatPomp10v11.rda"),{
  out <- readRDS("l8.rds")
...
stew(file=paste0(files_dir,"settings10v11.rda"),{
  ...
  if(run_level==1){
    ...
    pfilter_Np <- 12000
    abf_Nrep <- 1500
    abf_Np_per_replicate <- 200
    ubf_Nrep <- 23000
    abfir_Nrep <- 150
    abfir_Np_per_replicate <- 70
    abfir_Ninter <- 5
    enkf_Np <- 8000
    bpf_units_per_block <- 1
    bpf_Np <- 6000
...
spatPomp <- stew(file=paste0(files_dir,"spatPomp10v11.rda"),{
  load(file=paste0(li23_model_dir,"li23_spatPomp10v11.rda"))  
...
girf <- stew(file=paste0(files_dir,"girf10v11.rda"),seed=5981724,{
...
abf <- stew(file=paste0(files_dir,"abf10v11.rda"),seed=844424,{
...
ubf <- stew(file=paste0(files_dir,"ubf10v11.rda"),seed=844425,{
  foreach(job=iter(jobs,"row")) %do% {
    system.time(
      abf(li23_list[[job$U_id]], 
          Nrep = ubf_Nrep,
...
abfir <- stew(file=paste0(files_dir,"abfir10v11.rda"),seed=53398,{
  foreach(job=iter(jobs,"row")) %do% {
    system.time(
...
pfilter <- stew(file=paste0(files_dir,"pfilter10v11.rda"),seed=53286,{
  foreach(job=iter(jobs,"row")) %dopar% {
    system.time(
...
bpf <- stew(file=paste0(files_dir,"bpf10v11.rda"),seed=53285,{
  foreach(job=iter(jobs,"row")) %dopar% {
    system.time(
...
list_enkf <- li23_list
enkf <- stew(file=paste0(files_dir,"enkf10v11.rda"),seed=53285,{
  foreach(job=iter(jobs,"row")) %dopar% {
...
save(file=paste0(files_dir,"results10v11.rda"),results,jobs)
...
filter_tests <- ggplot(results,mapping = aes(x = U, y = logLik_per_obs, group=Method,color=Method,linetype=Method,shape=Method)) +
  ...
  coord_cartesian(ylim=c(max-6,max))+
...
ggsave("filter_tests10v11.png", plot=filter_tests, width=10, height=8, dpi=300)
