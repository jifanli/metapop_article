...
li23_spatPomp <- bake(file=paste0(li23_model_dir,"li23_spatPomp.rds"),{
  li23_U <- 373
  li23_N <- 30
...
bake(file=paste0(files_dir,"settings.rds"),{
...
spatPomp <- bake(file=paste0(files_dir,"spatPomp.rds"),{
...
girf <- bake(file=paste0(files_dir,"girf.rds"),seed=5981724,{
...
abf <- bake(file=paste0(files_dir,"abf.rds"),seed=844424,{
...
ubf <- bake(file=paste0(files_dir,"ubf.rds"),seed=844424,{
  foreach(job=iter(jobs,"row")) %do% {
    system.time(
      abf(list[[job$U_id]], 
          Nrep = ubf_Nrep,
...
abfir <- bake(file=paste0(files_dir,"abfir.rds"),seed=53398,{
  foreach(job=iter(jobs,"row")) %do% {
    system.time(
      abfir(list[[job$U_id]], 
...
pfilter <- bake(file=paste0(files_dir,"pfilter.rds"),seed=53285,{
  foreach(job=iter(jobs,"row")) %dopar% {
    system.time(
...
bpf <- bake(file=paste0(files_dir,"bpf.rds"),seed=53285,{
  foreach(job=iter(jobs,"row")) %dopar% {
    system.time(
...
list_enkf <- list
enkf <- bake(file=paste0(files_dir,"enkf.rds"),seed=53285,{
  foreach(job=iter(jobs,"row")) %dopar% {
...
save(file=paste0(files_dir,"results.rds"),results,jobs)
...
ggsave("filter_tests.png", plot=filter_tests, width=10, height=5, dpi=300)
