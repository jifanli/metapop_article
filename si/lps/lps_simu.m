...
num_times=30;
M(:,:,15:num_times)=0.2*repmat(M(:,:,14),[1,1,num_times-14]);
M(170,:,15:num_times)=0.02*repmat(M(170,:,14),[1,1,num_times-14]);
M(:,170,15:num_times)=0.02*repmat(M(:,170,14),[1,1,num_times-14]);
...
b=Td/a;
rnds=ceil(gamrnd(a,b,1e4,1));
Td=6;%average reporting delay
b=Td/a;
rndsaf=ceil(gamrnd(a,b,1e4,1));
pop=pop0;
simulation=zeros(num_loc,num_times);
x=zeros(num_loc*5+6,num_ens);
seedid=170;
x((seedid-1)*5+2,:)=1000;
x((seedid-1)*5+4,:)=1000;
C=M(:,170,1);%first day
for i=1:num_loc
    if i~=seedid
        ...
        x((i-1)*5+2,:)=round(C(i)*3*Ewuhan/pop0(seedid));
        %Ia
        Iawuhan=x((seedid-1)*5+4,:);
        x((i-1)*5+4,:)=round(C(i)*3*Iawuhan/pop0(seedid));
    end
end
for i=1:num_loc
    x((i-1)*5+1,:)=pop0(i);
end
obs_temp=zeros(num_loc,num_ens,num_times);
x(end-5:end,:)=[1.12;0.55;1.36;3.69;0.14;3.47];
for ts=1:14
    [x,pop]=SIMU(x,M,pop,ts,pop0);
    obs_cnt=H*x;
    for l=1:num_loc
                if obs_cnt(l,1)>0
                    rnd=datasample(rnds,obs_cnt(l,1));
                    for h=1:length(rnd)
                        if (ts+rnd(h)<=num_times)
                            obs_temp(l,1,ts+rnd(h))=obs_temp(l,1,ts+rnd(h))+1;
                        end
                    end
                end
            end
    simulation(:,ts)=obs_temp(:,:,ts);
end
x(end-5:end,:)=[0.35;0.43;1.36;3.42;0.69;3.31];
for ts=15:num_times
    [x,pop]=SIMU(x,M,pop,ts,pop0);
    obs_cnt=H*x;
    for l=1:num_loc
                if obs_cnt(l,1)>0
                    rnd=datasample(rndsaf,obs_cnt(l,1));
                    for h=1:length(rnd)
                        if (ts+rnd(h)<=num_times)
                            obs_temp(l,1,ts+rnd(h))=obs_temp(l,1,ts+rnd(h))+1;
                        end
...
writematrix(simulation, 'li_simu.csv');

function [x,pop]=SIMU(x,M,pop,ts,pop0)
