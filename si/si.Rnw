...
R0_uc_be <- readRDS("../profile8/R0/profile_R0_be_full.rds")[,c("logLik","R0_be1")]
# rbind(
#   readRDS("../profile8/R0/out_3/profile_R0_be.rds")[,c("logLik","R0_be1")],
#   readRDS("../profile8/R0/out_3/profile_R0_be2.rds")[,c("logLik","R0_be1")]
# )
  
R0_uc_af <- readRDS("../profile8/R0/profile_R0_af_full.rds")[,c("logLik","R0_af1")]
# R0_uc_af <- rbind(
#   readRDS("../profile8/R0/out_3/profile_R0_af.rds")[,c("logLik","R0_af1")],
 #   readRDS("../profile8/R0/out_3/profile_R0_af2.rds")[,c("logLik","R0_af1")]
 # )

Z_uc_be <-  readRDS("../profile8/Z/profile_Z_be_full.rds")[,c("logLik","Z_be1")]
# rbind(
#   readRDS("../profile8/Z/out_3/profile_Z_be3.rds")[,c("logLik","Z_be1")]
# )

Z_uc_af <-   readRDS("../profile8/Z/profile_Z_af_full.rds")[,c("logLik","Z_af1")]
# rbind(
#   readRDS("../profile8/Z/profile_Z_af_full.rds")[,c("logLik","Z_af1")]
# )

D_uc_be <-  readRDS("../profile8/D/profile_D_be_full.rds")[,c("logLik","D_be1")]
# rbind(
#   readRDS("../profile8/D/profile_D_be_full.rds")[,c("logLik","D_be1")]
# )

D_uc_af <-   readRDS("../profile8/D/profile_D_af_full.rds")[,c("logLik","D_af1")]
# rbind(
#   readRDS("../profile8/D/profile_D_af_full.rds")[,c("logLik","D_af1")]
# )

alpha_uc_be <- readRDS("../profile8/alpha/profile_alpha_be_full.rds")[,c("logLik","alpha_be1")]
# rbind(
#   readRDS("../profile8/alpha/out_3/profile_alpha_be2.rds")[,c("logLik","alpha_be1")],
#   readRDS("../profile8/alpha/out_3/profile_alpha_be3.rds")[,c("logLik","alpha_be1")]
# )
alpha_uc_be <- alpha_uc_be[alpha_uc_be$alpha_be1>0.02 & alpha_uc_be$alpha_be1<0.2 ,]

alpha_uc_af <-   readRDS("../profile8/alpha/profile_alpha_af_full.rds")[,c("logLik","alpha_af1")]
# rbind(
#   readRDS("../profile8/alpha/out_3/profile_alpha_af2.rds")[,c("logLik","alpha_af1")]
# )
alpha_uc_af <- alpha_uc_af[alpha_uc_af$alpha_af1>0.2 ,]

mu_uc_be <- rbind(
  readRDS("../profile8/mu/profile_mu_be_full.rds")[,c("logLik","mu_be1")]
)

mu_uc_af <- rbind(
  readRDS("../profile8/mu/profile_mu_af_full.rds")[,c("logLik","mu_af1")]
)  

Beta_uc_be <- rbind(
  readRDS("../profile8/Beta/profile_Beta_be_full.rds")[,c("logLik","Beta_be1")]
)
Beta_uc_be <- Beta_uc_be[Beta_uc_be$Beta_be1<1.25,]

Beta_uc_af <- rbind(
  readRDS("../profile8/Beta/profile_Beta_af_full.rds")[,c("logLik","Beta_af1")]
)

A_0_uc <- rbind(
  readRDS("../profile8/A_0/profile_A_0_full.rds")[,c("logLik","A_01")]
)
A_0_uc <- A_0_uc[A_0_uc$A_01<9000 ,]

E_0_uc <- rbind(
  readRDS("../profile8/E_0/profile_E_0_full.rds")[,c("logLik","E_01")]
)
E_0_uc <- E_0_uc[E_0_uc$E_01<9000 ,]

sigma_SE_uc <- rbind(
  readRDS("../profile8/sigma_SE/profile_sigma_SE_full.rds")[,c("logLik","sigma_SE1")]
)

tau_uc <- rbind(
  readRDS("../profile8/tau/profile_tau_full.rds")[,c("logLik","tau1")]
)
tau_uc <- tau_uc[tau_uc$tau1>0.2 & tau_uc$tau1<0.6 ,]

theta_uc <- rbind(
  readRDS("../profile8/theta/profile_theta_full.rds")[,c("logLik","theta1")]
)
...
d\Gamma_{\unit}/dt,
\\
\rate_{S_{\unit}T} \eqsep = \eqsep \rate_{E_{\unit}T} \eqsep = \eqsep \rate_{A_{\unit}T} &=& \mobilityFactor \sum_j \frac{M_{\unit j}}{\pop_{\unit}-I_{\unit}},
\\
\rate_{TS_{\unit}} &=& \mobilityFactor \sum_j \frac{M_{j \unit} S_{j}}{\pop_{j}-I_{j}},
\\
\rate_{TE_{\unit}} &=& \mobilityFactor \sum_j \frac{M_{j \unit} E_{j}}{\pop_{j}-I_{j}},
\\
\rate_{TA_{\unit}} &=&\mobilityFactor \sum_j \frac{M_{j \unit} A_{j}}{\pop_{j}-I_{j}},
\\
...
Equations (\ref{eq:dS})-(\ref{eq:muCbC}) specify an overdispersed continuous time Markov process via the limit of a discrete time Euler approximation as the discretization step approaches zero \citep{breto09,breto11}.
The parameters are described in Table~\ref{tab:parameters}.
...
<<table_params>>=
li1 <- li20(U=5,version='li20period1',for_ibpf=F)
li_par1 <- par_li(coef(li1),dir="from_li")
li3 <- li20(U=5,version='li20period3',for_ibpf=F)
li_par3 <- par_li(coef(li3),dir="from_li")
...
For {\RevisedModelConstrained}, $\latency^{\before}=\latency^{\after}$, so the two values marked by $^\ast$ are constrained to be equal; $^\dagger$ denotes the constraint $\infectiousPeriod^{\before}=\infectiousPeriod^{\after}$.
...
\begin{equation}
M_{uj} = M^{\mathrm {li20}}_{uj} + \frac{{\mathcal F} \, \bar{d}}{\bar P(t_0)}\times \frac{ P_u(t_0)  \, P_j(t_0)}{d_{uj}},
\end{equation}
where $M^{\mathrm {li20}}_{uj}$ is the movement from city $u$ to $j$ used by \citet{li20}, $P_u(t_0)$ is the initial population in city $u$; $\bar P(t_0)$ is the average population across all 373 cities; $d_{uj}$ is the distance from city $u$ to city $j$; $\bar d$ is the average of this distance over all $(373\times 372)/2$ pairs; $\mathcal F$ is a mobility correction factor which we took as ${\mathcal F}=20$ based on assessment of diagnostic plots.
...
Iterated particle filters suffer from the same curse of dimensiounality.
...
The recombination allows successful blocks of one particle to join up with different successful blocks from another particle.
...
\begin{figure}
\begin{center}
\includegraphics[width=15cm]{../filter_tests/filter_tests8.png}
\end{center}
\caption{Comparing filters on simulated data from model \code{li23}. ABF is the adapted bagged filter \citep{ionides21}; ABF-IR is the adapted bagged filter with intermediate resampling \citep{ionides21}; BPF is the block particle filter; EnKF is the ensemble Kalman filter; GIRF is the guided intermediate resampling filter \citep{park19}; PF is the particle filter; UBF is the unadapted bagged filter \citep{ionides21}.}\label{fig:filter_test}
\end{figure}
...
Two bagged filters (ABF and UBF) also perform well, however their structure is not well suited to parameter estimation via iterated perturbed filtering \citep{ionides21}.
...
Figure~\ref{fig:filter_test} is similar to Figure~3 of \citet{ionides21}, that shows an equivalent filter comparison for a longer collection of time series of an endemic viral disease, namely pre-vaccination measles.
Different relative performance results can be obtained on other model classes, for example in a linear Gaussian model (Figure~1 of \citet{ionides21}) or the Lorenz~96 geophysical model (Figure~S3 of \citet{ionides21}).
