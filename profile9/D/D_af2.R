...
idx <- setdiff(1:ncol(box), grep("D_af", colnames(box)))
profile_design(
  D_af1=seq(0.7,4.7,length=profile_pts),
...
bake(file =paste0(out_dir,"profile_D_af2.rds"), {
  
  cat(capture.output(sessionInfo()),
      file=paste0(out_dir,"sessionInfo_D_af2.txt"),sep="\n")
