...
idx <- setdiff(1:ncol(box), grep("tau", colnames(box)))
profile_design(
  tau1=seq(0.09,0.51,length=profile_pts),
...
bake(file =paste0(out_dir,"profile_tau2.rds"), {
  
  cat(capture.output(sessionInfo()),
      file=paste0(out_dir,"sessionInfo_tau2.txt"),sep="\n")
