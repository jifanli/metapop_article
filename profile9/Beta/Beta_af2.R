...
idx <- setdiff(1:ncol(box), grep("Beta_af", colnames(box)))
profile_design(
  Beta_af1=seq(0.15,0.45,length=profile_pts),
...
bake(file =paste0(out_dir,"profile_Beta_af2.rds"), {
  
  cat(capture.output(sessionInfo()),
      file=paste0(out_dir,"sessionInfo_Beta_af2.txt"),sep="\n")
