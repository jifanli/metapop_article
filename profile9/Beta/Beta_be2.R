...
idx <- setdiff(1:ncol(box), grep("Beta_be", colnames(box)))
profile_design(
  Beta_be1=seq(0.25,1.17,length=profile_pts),
...
bake(file =paste0(out_dir,"profile_Beta_be2.rds"), {
  
  cat(capture.output(sessionInfo()),
      file=paste0(out_dir,"sessionInfo_Beta_be2.txt"),sep="\n")
