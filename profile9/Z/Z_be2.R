...
basicParNames <- c("alpha_be","Beta_be","alpha_af","Beta_af",
...
idx <- setdiff(1:ncol(box), grep("Z_be", colnames(box)))
profile_design(
  Z_be1=seq(0.01,2.77,length=profile_pts),
...
bake(file =paste0(out_dir,"profile_Z_be2.rds"), {
  
  cat(capture.output(sessionInfo()),
      file=paste0(out_dir,"sessionInfo_Z_be2.txt"),sep="\n")
