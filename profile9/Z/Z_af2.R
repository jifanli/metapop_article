...
idx <- setdiff(1:ncol(box), grep("Z_af", colnames(box)))
profile_design(
  Z_af1=seq(0.01,15.7,length=profile_pts),
...
bake(file =paste0(out_dir,"profile_Z_af2.rds"), {
  
  cat(capture.output(sessionInfo()),
      file=paste0(out_dir,"sessionInfo_Z_af2.txt"),sep="\n")
