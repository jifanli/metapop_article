...
idx <- setdiff(1:ncol(box), grep("theta", colnames(box)))
profile_design(
  theta1=seq(0.1,4.44,length=profile_pts),
...
bake(file =paste0(out_dir,"profile_theta2.rds"), {
  
  cat(capture.output(sessionInfo()),
      file=paste0(out_dir,"sessionInfo_theta2.txt"),sep="\n")
