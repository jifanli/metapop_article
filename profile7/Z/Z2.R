...
idx <- setdiff(1:ncol(box), grep("Z", colnames(box)))
profile_design(
  Z1=seq(0.2,2.8,length=profile_pts),
...
bake(file =paste0(out_dir,"profile_Z2.rds"), {
  
  cat(capture.output(sessionInfo()),
      file=paste0(out_dir,"sessionInfo_Z2.txt"),sep="\n")
