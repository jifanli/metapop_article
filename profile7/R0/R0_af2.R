...
params_full <- readRDS("t1.rds")
params_full$Z_be1 <- params_full$Z1
params_full$Z_af1 <- params_full$Z1
params_full$D_be1 <- params_full$D1
params_full$D_af1 <- params_full$D1
params_full$R0_be1 <- R0(params_full, be=T)[,1]
params_full$R0_af1 <- R0(params_full, be=F)[,1]
params <- params_full[,c("logLik","logLik_se")]
params_full <- params_full[names(params_full)%in%names(coef(m1))] # remove Beta
...
idx <- setdiff(1:ncol(box), grep("R0_af", colnames(box)))
profile_design(
  R0_af1=seq(0.49,1.3,length=profile_pts),
...
basicParNames <- c("R0_af",basicParNames[idx])
...
bake(file =paste0(out_dir,"profile_R0_af2.rds"), {
  
  cat(capture.output(sessionInfo()),
      file=paste0(out_dir,"sessionInfo_R0_af2.txt"),sep="\n")
