...
idx <- setdiff(1:ncol(box), grep("D", colnames(box)))
profile_design(
  D1=seq(2.7,6.7,length=profile_pts),
...
bake(file =paste0(out_dir,"profile_D2.rds"), {
  
  cat(capture.output(sessionInfo()),
      file=paste0(out_dir,"sessionInfo_D2.txt"),sep="\n")
